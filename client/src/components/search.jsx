  ...
  return (
    <div className="flex flex-wrap justify-center gap-14 mt-10  w-2/3 mx-auto rounded-md p-5">
      {songs.map((song, index) => (
        <a href={`/play?id=${song.id}`} key={song.id || song.name}>
          <Card
            name={song.name}
            img={song.thumbnail}
            linesCount={song.lyrics.length}
            languages={usedLanguages(song.lyrics)}
            date={song.date}
...
export default function Search() {
  ...
  useEffect(() => {
    // getSongs(songQuery).then((songs) => setSongs(songs))
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        // const songs = await getSongs(searchInput, abortController.signal)
        const songs = await getSongs(searchInput, abortController.signal)
        setSongs(songs)
      } catch (e) {
      } finally {
